...
| 类 | 可见性 | 被谁使用 | OOP 角色 | 说明 |
|----|--------|---------|---------|------|
| `relay_active_level_t` | 用户 API | main.c | 枚举 | 继电器有效电平 |
| `relay_source_t` | 用户 API | 所有调用 set/toggle 的模块 | 枚举 | 操作来源标签 |
| `relay_config_t` | 用户 API | main.c | 配置结构体 | 创建时的 GPIO + 电平参数 |
| `relay_t` | 用户 API (opaque) | main.c + app_controller | 句柄 | 继电器实例，struct 定义在 .c |
...
**所属层**：驱动适配层
**可见性**：用户 API — `main.c` 从 board_pinmap 读取后填入
**OOP 角色**：配置结构体
...
**所属层**：驱动适配层
**可见性**：用户 API (opaque) — `main.c` 创建后注入 app_controller；struct 定义在 `.c` 中
**OOP 角色**：句柄
...
**关键设计决策**：
- 配置接受 `gpio_num_t`，不直接依赖 `board_pinmap` — 由 `main.c` 从 pinmap 读取后填入，保持 relay 的可移植性和可测试性
...
| 类 | 可见性 | 被谁使用 | OOP 角色 | 说明 |
|----|--------|---------|---------|------|
| `button_active_level_t` | 用户 API | main.c | 枚举 | 按键有效电平 |
| `button_event_t` | 用户 API | app_controller 回调 | 枚举 | 按键事件类型 |
| `button_config_t` | 用户 API | main.c | 配置结构体 | 创建时的 GPIO + 电平参数 |
| `button_event_cb_t` | 用户 API | app_controller | 回调类型 | 单个事件的回调签名 |
| `button_t` | 用户 API (opaque) | main.c + app_controller | 句柄 | 按键实例，struct 定义在 .c |
...
- 不依赖 `board_pinmap` — GPIO 和电平由 `main.c` 从 pinmap 读取后填入 `button_config_t`
- `button_destroy` 内部注销所有 iot_button 回调并释放 iot_button 对象
- 当前只使用 GPIO backend，`button_create()` 已校验 GPIO 有效；ESP-IDF 6.0 的 `gpio_reset_pin()` 对有效 GPIO 返回 `ESP_OK`，因此正常调用图中底层 delete 失败不可达
- `espressif/button` 4.1.6 的 GPIO delete 会先释放 driver，再透传 `gpio_reset_pin()` 错误；若未来 backend 可返回 deinit 错误，底层可能进入不可重试的部分销毁状态，升级组件或扩展 backend 时必须重新核对 delete ownership 契约
- `button_destroy()` 的 `ESP_ERR_TIMEOUT` 发生在底层 delete 前，保留句柄并允许重试；底层 delete 的其它异常错误不承诺句柄仍可用
...
`relay` 和 `button` 接受 `gpio_num_t` 配置，由 `main.c` 从 `board_pinmap` 读取后创建并注入 app_controller，保持各模块的独立可测试性。
...
| 类 | 可见性 | 被谁使用 | OOP 角色 | 说明 |
|----|--------|---------|---------|------|
| `bl0942_t` | 用户 API (opaque) | main.c + app_controller | 句柄 | BL0942 实例，struct 定义在 .c |
| `bl0942_config_t` | 用户 API | main.c | 配置结构体 | UART + GPIO + 波特率 + 采样参数 |
...
**所属层**：驱动适配层
**可见性**：用户 API — `main.c` 从 board_pinmap 读取 UART/GPIO 后填入
**OOP 角色**：配置结构体
...
**所属层**：驱动适配层
**可见性**：用户 API (opaque) — `main.c` 创建后注入 app_controller，metering_service 通过事件消费数据；struct 定义在 `.c` 中
**OOP 角色**：句柄
...
```c
/* main.c 装配（创建子类，转为基类指针注入 network_manager） */
...
`bl0942` 和 `relay`/`button` 一样不直接依赖 `board_pinmap` — GPIO 和 UART 参数由 `main.c` 从 pinmap 读取后填入 `bl0942_config_t`。
...
| 类 | 可见性 | 被谁使用 | OOP 角色 | 说明 |
|----|--------|---------|---------|------|
| `metering_service_t` | 用户 API (opaque) | main.c + app_controller | 句柄 | struct 定义在 .c |
| `metering_config_t` | 用户 API | main.c | 配置结构体 | 工程量转换系数 |
| `metering_snapshot_t` | 用户 API | safety_guard + thingsboard_client + lvgl_dashboard | 值对象 | 单次采样工程量快照 |

### 7.2 `metering_config_t` — 初始化配置

**所属层**：业务服务层
**可见性**：用户 API — `main.c` 在创建时传入
**OOP 角色**：配置结构体
...
**所属层**：业务服务层
**可见性**：用户 API (opaque) — `main.c` 创建后注入 app_controller，consumers 通过事件订阅消费数据；struct 定义在 `.c` 中
**OOP 角色**：句柄
...
- `stop()` 在锁外注销事件 handler，收尾阶段必须重新获得 mutex 后再提交 handler/状态变化；若无限等待被 `xTaskAbortDelay()` 中断，继续取锁完成状态恢复；此前无其它清理错误时返回 `ESP_ERR_TIMEOUT`
- `destroy()` 采用非消费式失败语义：仅返回 `ESP_OK` 时释放句柄；stop 或 handler 清理失败时保留对象和未完成状态，调用方在外部串行化后重试 stop/destroy
...
| 类 | 可见性 | 被谁使用 | OOP 角色 | 说明 |
|----|--------|---------|---------|------|
| `wifi_link_config_t` | 用户 API | main.c | 配置结构体 | Wi-Fi + MQTT 连接参数 |
...
| 类 | 可见性 | 被谁使用 | OOP 角色 | 说明 |
|----|--------|---------|---------|------|
| `network_manager_t` | 用户 API (opaque) | main.c + app_controller + thingsboard_client | 句柄 | struct 定义在 .c |
| `network_manager_config_t` | 用户 API | main.c | 配置结构体 | 主/备链路指针 + 切换策略参数 |
| `network_manager_status_t` | 用户 API | app_controller + thingsboard_client | 值对象 | 聚合状态快照 |

### 9.2 `network_manager_config_t` — 初始化配置

**所属层**：网络抽象层
**可见性**：用户 API — `main.c` 创建时传入
**OOP 角色**：配置结构体
...
| 类 | 可见性 | 被谁使用 | OOP 角色 | 说明 |
|----|--------|---------|---------|------|
| `safety_guard_t` | 用户 API (opaque) | main.c + app_controller | 句柄 | struct 定义在 .c |
| `safety_guard_config_t` | 用户 API | main.c | 配置结构体 | 过流/过功率阈值 + 持续判定参数 |
...
| 类 | 可见性 | 被谁使用 | OOP 角色 | 说明 |
|----|--------|---------|---------|------|
| `tft_panel_t` | 用户 API (opaque) | main.c + app_controller + lvgl_dashboard | 句柄 | struct 定义在 .c |
| `tft_panel_config_t` | 用户 API | main.c | 配置结构体 | SPI 引脚 + 面板尺寸 + 背光 GPIO |
...
| 类 | 可见性 | 被谁使用 | OOP 角色 | 说明 |
|----|--------|---------|---------|------|
| `lvgl_dashboard_t` | 用户 API (opaque) | main.c + app_controller | 句柄 | struct 定义在 .c |
| `lvgl_dashboard_config_t` | 用户 API | main.c | 配置结构体 | tft_panel 句柄 + LVGL 任务参数 |
...
| 类 | 可见性 | 被谁使用 | OOP 角色 | 说明 |
|----|--------|---------|---------|------|
| `thingsboard_client_t` | 用户 API (opaque) | main.c + app_controller | 句柄 | struct 定义在 .c |
| `tb_client_config_t` | 用户 API | main.c | 配置结构体 | TB 平台参数 + network_manager 句柄 |
...
| 类 | 可见性 | 被谁使用 | OOP 角色 | 说明 |
|----|--------|---------|---------|------|
| `lte_link_config_t` | 用户 API | main.c | 配置结构体 | UART/GPIO + APN + MQTT 参数 |
...
- `stop()` 在锁外停止借用模块并注销回调，生命周期标志读写和收尾提交均不可因 mutex 等待中断而跳过；收尾等待被中断且此前无其它清理错误时返回 `ESP_ERR_TIMEOUT`
- `destroy()` 不销毁配置中借用的模块句柄，并采用非消费式失败语义：仅返回 `ESP_OK` 时释放 controller；失败时调用方必须保持所有借用句柄有效，并在外部串行化后重试 stop/destroy
